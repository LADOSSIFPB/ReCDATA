...
import br.edu.ifpb.recdata.widgets.AlertsDialogs.CadastroUsuarioAlertDialog;
...
public class TelaCadastraUsuario extends Activity implements OnClickListener {
	...
	public void onClick(View v) {
		...
		if ((Validacao.validaNome(nome))
			&&(Validacao.validaCPF(removeMascarar(cpf), cpf))
			  && (Validacao.validaCampo(login))
				&& (Validacao.validaSpinner(tipoUsuarioSpinner.getSelectedItem().toString(), this))
				  &&(Validacao.validaCampo(dataNascimento))
					&&(Validacao.validaTelefone(telefone))
					 &&(Validacao.validaEndereco(endereco))
				       &&(Validacao.validaEmail(email))
					     &&(Validacao.validaSenha(senha, senhaConfirma))
				){
			CadastroUsuarioAlertDialog cadastroUsuarioAlertDialog = new CadastroUsuarioAlertDialog(this);
			cadastroUsuarioAlertDialog.showAlertDialog();
		}else{
			getViews();
		}
	}
	...
	public String removeMascarar(EditText campo) {
		String s;
	...
	public JSONObject montarObjetoJSON() {
		...
		try {
			usuarioJsonObject.put("nome", nome.getText().toString().trim());
			usuarioJsonObject.put("email", email.getText().toString().trim());
			usuarioJsonObject.put("telefone", removeMascarar(telefone).trim());
			usuarioJsonObject.put("nascimento",
					getValorEditTextDataNascimento().trim());
			usuarioJsonObject.put("sexo", capturavalorRadioGroup());
			usuarioJsonObject.put("cpf", removeMascarar(cpf).trim());
			usuarioJsonObject.put("endereco", endereco.getText().toString().trim());
			usuarioJsonObject.put("login", login.getText().toString().trim());
			usuarioJsonObject.put("senha", senha.getText().toString().trim());
	...
	private void getViews() {
		...
		this.senhaConfirma = (EditText) findViewById(R.id.confirma_senha);
